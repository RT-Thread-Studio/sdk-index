...
from ci_config import CSP_NANO_VERSION
from ci_config import CSP_RTT_VERSION
para_json_tmp = """
{
 "parameter": {
   "csp_path": "$csp_path",
...
def gen_sdk_para_json_file(csp_path, output_project_path):
    init_logger()
    rtt_nano_path = os.path.join(csp_path, "sdk-rt-thread-nano-source-code-"+CSP_NANO_VERSION)
    rtt_path = os.path.join(csp_path, "sdk-rt-thread-source-code-"+CSP_RTT_VERSION)
    toolchain_name = "gcc"
    try:
        ...
        with open(csp_path+"/csp_chips.json", "w", encoding="UTF8") as f:
